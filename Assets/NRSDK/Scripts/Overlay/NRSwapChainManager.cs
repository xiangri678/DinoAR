...
namespace NRKernal
{
    ...
    public struct ViewPort
    {
        ...
        public override string ToString()
        {
            return string.Format("[index:{0} nativeHandler:{1} swapchainHandler:{2} targetDisplay:{3} viewportType:{4} spaceType:{5} is3DLayer:{6} isExternalSurface:{7} textureArraySlice:{8} nativePose:{9}]",
                index, nativeHandler, swapchainHandler, targetDisplay, viewportType, spaceType, is3DLayer, isExternalSurface, textureArraySlice, nativePose);
        }
    };
    ...
    public class NRSwapChainManager : SingletonBehaviour<NRSwapChainManager>
    {
        ...
        void InitDisplayOverlay()
        {
            ...
            NRDebugger.Info("[SwapChain] InitDisplayOverlay MonoMode");
            ...
            {
                NRDebugger.Info("[SwapChain] InitDisplayOverlay StereoMode.");
        ...
        void OnPreRenderImp(Camera camera)
        {
            ...
            if (NRFrame.MonoMode || camera == m_ShadowCamera || (m_LeftDisplayOverlay != null && camera == m_LeftDisplayOverlay.renderCamera))
                PopulateFrame();
        }
#endif
        ...
        internal void PopulateFrame()
        {
            ...
            if (m_MonoDisplayOverlay != null && !m_MonoDisplayOverlay.isReady)
                return;
#endif
            ...
            if (NRDebugger.logLevel <= LogLevel.Debug)
                NRDebugger.Info("[SwapChain] PopulateFrame");
        ...
        private void AddLayer(OverlayBase overlay)
        {
            ...
            if (!(overlay is NRDisplayOverlay))
                isExternalSurface = overlay.isExternalSurface;
        ...
        private List<UInt64> GetDynamicLayerHandlers()
        {
            ...
            for (int i = 0; i < Overlays.Count; i++)
            {
                ...
                if (overlay.IsActive)
                {
                    if (overlay is NRDisplayOverlay)
                    {
                        var displayOverlay = overlay as NRDisplayOverlay;
                        if (displayOverlay.isReady)
                            mDynLayers.Add(overlay.SwapChainHandler);
                    }
                    else if (overlay.isDynamic)
                    {
                        mDynLayers.Add(overlay.SwapChainHandler);
                    }
                }
