    ...
    internal class NativeMeshing
    {
        ...
        public bool RequestMeshInfo(Func<BlockInfo, bool> predicate = null)
        {
            ...
            for (int i = 0; i < blockListSize; i++)
            {
                ...
                result = NativeApi.NRMeshingBlockGetBlockIdentifier(PerceptionHandle, meshing_block_handle, ref blockInfo.identifier);
                NRDebugger.Debug("[NativeMeshing] NRMeshingBlockGetBlockIdentifier result: {0} identifier: {1}.", result, blockInfo.identifier);
                ...
                result = NativeApi.NRMeshingBlockGetBlockState(PerceptionHandle, meshing_block_handle, ref blockInfo.blockState);
                NRDebugger.Debug("[NativeMeshing] NRMeshingBlockGetBlockState result: {0} blockState: {1}.", result, blockInfo.blockState);
        ...
        /// </summary>
        /// <returns> True if it succeeds, false if it fails. </returns>
        public IEnumerator GetMeshInfoData(Action<ulong, NRMeshingBlockState, Mesh> action)
        {
            if (PerceptionHandle == 0)
            {
            ...
            foreach (var item in m_BlockInfos)
            {
                ...
                if (vertexCount != 0)
                {
                    ...
                    action?.Invoke(item.Value.identifier, item.Value.blockState, mesh);
        ...
        private partial struct NativeApi
        {
            ...
            /// <returns> The result of operation. </returns>
            [DllImport(NativeConstants.NRNativeLibrary)]
            public static extern NativeResult NRMeshingBlockGetBlockIdentifier(UInt64 perception_handle, UInt64 meshing_block_handle, ref ulong out_block_identifier);
            ...
            /// <returns> The result of operation. </returns>
            [DllImport(NativeConstants.NRNativeLibrary)]
            public static extern NativeResult NRMeshingBlockGetBlockState(UInt64 perception_handle, UInt64 meshing_block_handle, ref NRMeshingBlockState out_block_state);
            ...
            /// <returns> The result of operation. </returns>
            [DllImport(NativeConstants.NRNativeLibrary)]
            public static extern NativeResult NRMeshingBlockGetIndeices(UInt64 perception_handle, UInt64 meshing_block_handle, ushort[] out_face_vertices_index);
